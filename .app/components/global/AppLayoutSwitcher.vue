...
const currentPrimary: Ref<string> = ref('sky')
const currentMuted: Ref<string> = ref('slate')
...
const switchPrimary = (color: (typeof primaryPresets)[number]): void => {
  currentPrimary.value = color.name
  switchColorShades('primary', color.shades)
}
const switchMuted = (color: (typeof mutedPresets)[number]): void => {
...
<template>
  ...
  >
    ...
    <div class="nui-slimscroll max-h-[550px] overflow-y-auto px-4 pb-4 md:px-6 md:pb-6">
